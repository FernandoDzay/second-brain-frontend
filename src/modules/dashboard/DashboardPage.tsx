...
const DashBoardPage: React.FC<Props> = () => {
    const getMonthPaymentsSummaryQuery = useGetMonthPaymentsSummary();

    return (
        <div className="*:data-[slot=card]:from-primary/5 *:data-[slot=card]:to-card dark:*:data-[slot=card]:bg-card grid grid-cols-1 gap-4 px-4 *:data-[slot=card]:bg-gradient-to-t *:data-[slot=card]:shadow-xs lg:px-6 @xl/main:grid-cols-2 @5xl/main:grid-cols-4">
            <Card className="@container/card">
                <CardHeader>
                    <CardDescription>Total gastado en la quincena</CardDescription>
            ...
            <Card className="@container/card">
                <CardHeader>
                    <CardDescription>Total ingresos en la quincena</CardDescription>
                    <CardTitle className="text-2xl font-semibold tabular-nums @[250px]/card:text-3xl">
                        {getMonthPaymentsSummaryQuery.isFetching ? (
                            <Skeleton className="h-[36px]" />
                        ) : getMonthPaymentsSummaryQuery.isError ? (
